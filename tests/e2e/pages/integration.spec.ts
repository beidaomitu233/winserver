...
test.describe('Cross-Page Integration', () => {
  ...
  test('Sidebar nav items', async ({ page }) => { const items = page.locator('button.nav-item'); const count = await items.count(); await recordStep(page, P, 'Nav items', count >= 6, 'Found ' + count) })
  test('Files module is removed', async ({ page }) => { const filesNav = page.locator('button.nav-item').filter({ hasText: '文件' }); const count = await filesNav.count(); await recordStep(page, P, 'Files nav removed', count === 0, count === 0 ? 'Removed' : 'Still visible'); expect(count).toBe(0) })
  test('Navigate all pages sequentially', async ({ page }) => { const pages = ['dashboard', 'sites', 'database', 'software', 'logs', 'settings']; let allOK = true; for (const p of pages) { const ok = await navigateToPage(page, p); await page.waitForTimeout(400); await recordStep(page, P, 'Nav ' + p, ok, ok ? 'OK' : 'Failed'); if (!ok) allOK = false } expect(allOK).toBeTruthy() })
