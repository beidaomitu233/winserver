...
test.describe('Logs Page', () => {
  ...
  test('Logs auto-refresh toggle', async ({ page }) => { const btn = page.locator('button:has-text("Auto")').first(); const e = await btn.isVisible().catch(() => false); await recordStep(page, P, 'Auto-refresh', e, e ? 'Found' : 'Missing'); if (e) { await btn.click(); await page.waitForTimeout(200); await btn.click() } })
  test('Logs manual refresh', async ({ page }) => { const btn = page.locator('button:has-text("Refresh")').first(); if (await btn.isVisible().catch(() => false)) { await btn.click(); await page.waitForTimeout(500); await recordStep(page, P, 'Refreshed', true, 'OK') } })
